...
func SimpleAppGenTx(cdc *codec.Codec, pk crypto.PubKey) (
	...
	addr, secret, errCoin := server.GenerateCoinKey()
	if errCoin != nil {
		return
	}

	appGenTx, err = cdc.MarshalJSON(struct {
	...
	cliPrint, err = cdc.MarshalJSON(map[string]string{"secret": secret})
	if err != nil {
		return
	}
...
`),
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if !common.FileExists(genFile) {
				return fmt.Errorf("%s does not exist, run `gaiad ini` first", genFile)
			}
			genContexts, err := ioutil.ReadFile(genFile)
			if err != nil {
				return err
			}
			if err = cdc.UnmarshalJSON(genContexts, &genDoc); err != nil {
				return err
			}
			...
			for _, stateAcc := range appState.Accounts {
				if stateAcc.Address.Equals(addr) {
					return fmt.Errorf("the application state alrealy contains account %v", addr)
				}
			}
...
func appExporter() server.AppExporter {
	return func(logger log.Logger, db dbm.DB, _ io.Writer, _ int64, _ bool, _ []string) (
		...
		return app.NewNameServiceApp(logger, db).ExportAppStateAndValidators()
	}
}
